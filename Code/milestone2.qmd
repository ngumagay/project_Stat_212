...
all50against %>%
  ...
  geom_col(aes(fill = avgbatspeeddiff > 0)) +  # Conditional color fill based on avgbatspeeddiff > 0
  geom_hline(yintercept = -1.465536e-16, color = 'blue', size = 1.5) +
  ...
  scale_fill_manual(values = c("red", "darkgreen"), name = "Bat Speed Difference", labels = c("Below Average", "Above Average"))  # Red for below 0, green for above
```
```{r}
